...
def get_completion_old(prompt):
    response = openai.Completion.create(  # BREAKING: Completion.create deprecated, use chat.completions.create
...
def get_chat_old(messages):
    response = openai.ChatCompletion.create(  # BREAKING: ChatCompletion.create deprecated
        model="gpt-3.5-turbo",
        messages=messages
    )
...
def get_embedding_old(text):
    response = openai.Embedding.create(  # BREAKING: Embedding.create deprecated
